...
class SingleStageDetector(nn.Module):
    ...
    def forward(self,
        ...
        #torch.cuda.synchronize()
        #ed = time.time()
        #print('3D backbone', (ed-st) * 1000, [(k, v[0].shape[0]) for k, v in x.kmaps.items()])#x.F.shape, [(k, v[1]) for k, v in x.kmaps.items()], [(k, v[0].shape[0]) for k, v in x.kmaps.items()])
        #print('3D backbone', (ed-st) * 1000, [(k, torch.sum(v[3]).item()) for k, v in features.indice_dict.items()])
